...
class TestEnhancedWorkflowMcp(unittest.IsolatedAsyncioTestCase):
    """
    enhanced_workflow_mcp 单元测试类
    ...
    async def asyncTearDown(self):
        ...
        # 生成测试报告
        test_report = {
            'test_id': f'MCP_TestEnhancedWorkflowMcp_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
            'test_name': 'TestEnhancedWorkflowMcp',
            'module_name': self.module_name,
        ...
        # 保存测试报告
        report_path = Path(__file__).parent.parent / f'test_report_enhanced_workflow_mcp_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
...
def run_tests():
    ...
    # 运行测试套件
    suite = unittest.TestLoader().loadTestsFromTestCase(TestEnhancedWorkflowMcp)
