...
# Mock模块导入处理
try:
    from directory_structure_mcp.directory_structure_mcp import DirectoryStructureMcp
    MOCK_MODE = False
    print("成功导入真实模块")
    ...
    class DirectoryStructureMcp:
        def __init__(self, *args, **kwargs):
            self.name = "directory_structure_mcp"
            self.status = "running"
...
class TestDirectoryStructureMcpComprehensive(IsolatedAsyncioTestCase):
    """
    DirectoryStructureMcp 完整测试套件
    """
    ...
    def setUp(self):
        ...
        # 创建测试实例
        try:
            self.test_instance = DirectoryStructureMcp()
