...
# Mock模块导入处理
try:
    from enterprise_smartui_mcp.enterprise_smartui_mcp import EnterpriseSmartuiMcp
    MOCK_MODE = False
    print("成功导入真实模块")
except ImportError as e:
    print(f"导入错误: {e}")
    # 创建Mock类
    class EnterpriseSmartuiMcp:
        def __init__(self, *args, **kwargs):
            self.name = "enterprise_smartui_mcp"
            self.status = "running"
...
class TestEnterpriseSmartuiMcpComprehensive(IsolatedAsyncioTestCase):
    """
    EnterpriseSmartuiMcp 完整测试套件
    """
    ...
    def setUp(self):
        ...
        # 创建测试实例
        try:
            self.test_instance = EnterpriseSmartuiMcp()
