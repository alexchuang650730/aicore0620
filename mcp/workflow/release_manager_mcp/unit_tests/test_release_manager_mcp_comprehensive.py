...
# Mock模块导入处理
try:
    from release_manager_mcp.release_manager_mcp import ReleaseManagerMcp
    MOCK_MODE = False
    print("成功导入真实模块")
...
class TestReleaseManagerMcpComprehensive(IsolatedAsyncioTestCase):
    """
    ReleaseManagerMcp 完整测试套件
    """
    ...
    def setUp(self):
        ...
        # 创建测试实例
        try:
            self.test_instance = ReleaseManagerMcp()
