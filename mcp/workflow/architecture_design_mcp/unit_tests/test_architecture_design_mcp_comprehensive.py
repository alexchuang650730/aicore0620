...
# Mock模块导入处理
try:
    from architecture_design_mcp.architecture_design_mcp import ArchitectureDesignMcp
    MOCK_MODE = False
    print("成功导入真实模块")
...
class TestArchitectureDesignMcpComprehensive(IsolatedAsyncioTestCase):
    """
    ArchitectureDesignMcp 完整测试套件
    """
    ...
    def setUp(self):
        ...
        # 创建测试实例
        try:
            self.test_instance = ArchitectureDesignMcp()
